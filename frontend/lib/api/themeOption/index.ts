...
import { tThemeListApiResponse } from "@/lib/api/themeOption/type";

export default async function getTheme(): Promise<ThemeOptions> {
  try {
    const base = process.env.NEXT_PUBLIC_MAP_API_THEME;
    const params = process.env.NEXT_PUBLIC_MAP_API_THEME_PARAMS;

    if (!base || !params) {
      console.warn("[getTheme] API env missing, skip fetch");
      return {} as ThemeOptions;
    }

    const dataSetting: tThemeListApiResponse = await getFetch(
      `${base}?${params}`
    );
 ...
 */
function convert(data: tThemeListApiResponse): ThemeOptions {
  try {
    ...
    data.listContent.forEach((item) => {
      ...
      const key = item.content_items.find((c) => c.label === "項目名")
        ?.raw_value as string;
    ...
    console.error("getTheme convert", e);
    return {};
  }
}
